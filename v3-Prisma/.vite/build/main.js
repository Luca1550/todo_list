...
function createBetterSQLite3Client(input) {
	...
	const db = new better_sqlite3.default(url.replace(/^file:/, ""), config);
	db.defaultSafeIntegers(true);
	return db;
}
//#endregion
//#region node_modules/@prisma/client-runtime-utils/dist/index.js
	...
	function getBacktrace(log3) {
		if (log3.fields?.message) {
			...
			if (log3.fields?.file) {
				...
				if (log3.fields?.line) str += `:${log3.fields.line}`;
				if (log3.fields?.column) str += `:${log3.fields.column}`;
			}
			if (log3.fields?.reason) str += `
${log3.fields?.reason}`;
			return str;
		}
	...
	var ObjectEnumValue = class {
		...
		#representation;
		constructor(arg) {
			if (arg === secret) this.#representation = `Prisma.${this._getName()}`;
			else this.#representation = `new Prisma.${this._getNamespace()}.${this._getName()}()`;
		}
		_getName() {
	...
	function checkInt32(i, min2, max2) {
		if (i !== ~~i || i < min2 || i > max2) throw Error(invalidArgument + i);
	}
	...
	var divide = /* @__PURE__ */ function() {
		...
		return function(x, y, pr, rm, dp, base) {
			...
			if (!xd || !xd[0] || !yd || !yd[0]) return new Ctor(!x.s || !y.s || (xd ? yd && xd[0] == yd[0] : !yd) ? NaN : xd && xd[0] == 0 || !yd ? sign2 * 0 : sign2 / 0);
			if (base) {
				logBase = 1;
				e = x.e - y.e;
			} else {
	...
	function naturalExponential(x, sd) {
		...
		for (;;) {
			...
			if (digitsToString(t.d).slice(0, wpr) === digitsToString(sum2.d).slice(0, wpr)) {
				...
				if (sd == null) if (rep < 3 && checkRoundingDigits(sum2.d, wpr - guard, rm, rep)) {
					Ctor.precision = wpr += 10;
					denominator = pow2 = t = new Ctor(1);
					i = 0;
					rep++;
				} else return finalise(sum2, Ctor.precision = pr, rm, external = true);
				else {
					Ctor.precision = pr;
					return sum2;
				}
	...
	function naturalLogarithm(y, sd) {
		...
		for (;;) {
			...
			if (digitsToString(t.d).slice(0, wpr) === digitsToString(sum2.d).slice(0, wpr)) {
				...
				if (sd == null) if (checkRoundingDigits(sum2.d, wpr - guard, rm, rep)) {
					Ctor.precision = wpr += guard;
	...
	function clone(obj) {
		...
		function Decimal2(v) {
			...
			if (isDecimalInstance(v)) {
				x.s = v.s;
				if (external) if (!v.d || v.e > Decimal2.maxE) {
					x.e = NaN;
					x.d = null;
			...
			if (t === "number") {
				...
				if (v === ~~v && v < 1e7) {
					...
					if (external) if (e > Decimal2.maxE) {
						x.e = NaN;
						x.d = null;
	...
	function join(values, separator = ",", prefix = "", suffix = "") {
		if (values.length === 0) throw new TypeError("Expected `join([])` to be called with an array of multiple elements, but got an empty array");
		return new Sql([
			prefix,
...
*/
//#endregion
//#region src/prisma/generated/internal/class.ts
var import_client = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var q = (e, t) => () => (t || e((t = { exports: {} }).exports, t), t.exports), Ne = (e, t) => {
	...
	var B = (e, t, r) => (r = e != null ? nu(su(e)) : {}, xi(t || !e || !e.__esModule ? Qt(r, "default", {
		value: e,
		enumerable: !0
	}) : r, e)), lu = (e) => xi(Qt({}, "__esModule", { value: !0 }), e);
	var Fi = q((Af, Iu) => {
		Iu.exports = {
	...
	var Li = q((Rf, $i) => {
		...
		$i.exports = (e) => {
			let t = e.match(/^[ \t]*(?=\S)/gm);
			return t ? t.reduce((r, n) => Math.min(r, n.length), Infinity) : 0;
		};
	});
	var Bi = q((Of, Ui) => {
		"use strict";
	...
	var da = q((wx, he) => {
		...
		var la = (e = "") => aa(ld(e)).toString(36).slice(1), oa = Array.from({ length: 26 }, (e, t) => String.fromCharCode(t + 97)), ud = (e) => oa[Math.floor(e() * oa.length)], ua = ({ globalObj: e = typeof global < "u" ? global : typeof window < "u" ? window : {}, random: t = Math.random } = {}) => {
			let r = Object.keys(e).toString();
			return la(r.length ? r + _n(Nt, t) : _n(Nt, t)).substring(0, Nt);
		}, ca = (e) => () => e++, cd = 476782367, pa = ({ random: e = Math.random, counter: t = ca(Math.floor(e() * cd)), length: r = sa, fingerprint: n = ua({ random: e }) } = {}) => function() {
			let o = ud(e), s = Date.now().toString(36), a = t().toString(36);
			return `${o + la(`${s + _n(r, e) + a + n}`).substring(1, r)}`;
...
`);
	...
	var Hi = /* @__PURE__ */ new Set(), ln = (e, t, ...r) => {
			...
			clone: function e(t, r) {
				...
				switch (r = r || {}, o) {
					case "Object":
						...
						return n;
					case "Array": return i = h.util.objId(t), r[i] ? r[i] : (n = [], r[i] = n, t.forEach(function(s, a) {
						n[a] = e(s, r);
					}), n);
					default: return t;
				}
	...
	var Yt = class e {
		...
		slice(t, r) {
			return new e(t, Ki(this.lines.slice(t - 1, r).join(`
`)).split(`
`));
		}
		highlight() {
...
`);
	...
	function ho(e) {
		...
		if (e instanceof Date) return new I(`new Date("${Kt(e) ? e.toISOString() : "Invalid Date"}")`);
	...
	function sr(e, t) {
		...
		return {
			...
			args: new $e(0, { colors: r }).write(e).toString()
		};
	}
	...
	function xc(e) {
		let t = new Y(), r = (n, i) => t.getOrCreate(n, () => i.has(n) ? [n] : (i.add(n), e[n] ? e[n].needs.flatMap((o) => r(o, i)) : [n]));
		return Wt(e, (n) => ({
			...n,
	...
	function Ac(e, t, r) {
		let n = r.getComputedFields(), o = Po({
			...r.getGlobalOmit(),
			...t
		}, n);
	...
	var gn = /* @__PURE__ */ new WeakMap(), dr = "$$PrismaTypedSql", Pt = class {
		constructor(t, r) {
			gn.set(this, {
				sql: t,
				values: r
	...
	function W(e, t) {
		let r = Nc(t), n = /* @__PURE__ */ new Set(), i = new Proxy(e, {
			get(o, s) {
...
`).reduce(function(r, n) {
	...
	function jo(e, t) {
		let n = Si(t.fields.filter((i) => !i.relationName), "name");
		return new Proxy({}, {
			get(i, o) {
	...
	function Ho(e) {
		...
		return vt(Object.create(this._originalClient, {
	...
	function es(e, t) {
		...
		return Xo(e, t, e._extensions.getAllQueryCallbacks(r ?? "$none", o));
	}
	function ts(e) {
		return (t) => {
	...
	function hp(e) {
		switch (e.cause.kind) {
			...
			case "mysql":
			case "mssql": return;
			default: T(e.cause, `Unknown error: ${K(e.cause)}`);
		}
	}
	function ds(e) {
		switch (e.cause.kind) {
	...
	function vp(e) {
		...
		return t.length === 0 ? "" : (t.sort(([n], [i]) => n.localeCompare(i)), `/*${t.map(([n, i]) => {
			return `${encodeURIComponent(n)}='${encodeURIComponent(i).replace(/'/g, "\\'")}'`;
		}).join(",")}*/`);
	}
	function br(e, t) {
		let r = {};
	...
	async function xr({ query: e, tracingHelper: t, provider: r, onQuery: n, execute: i }) {
		let o = n === void 0 ? i : async () => {
			let s = /* @__PURE__ */ new Date(), a = performance.now(), l = await i();
			return n({
				timestamp: s,
	...
	function wa(e, t) {
		let r = t || Ed();
		return Sd(!e || isNaN(e) ? Date.now() : e, ha) + Td(bd, r);
	}
	var D = [];
	...
	var Vn = { randomUUID: require("node:crypto").randomUUID };
	function Ad(e, t, r) {
	...
	function Rd(e, t, r) {
		return e.msecs ??= -Infinity, e.seq ??= 0, t > e.msecs ? (e.seq = r[6] << 23 | r[7] << 16 | r[8] << 8 | r[9], e.msecs = t) : (e.seq = e.seq + 1 | 0, e.seq === 0 && e.msecs++), e;
	}
	...
	function Xn(e, t, r, n) {
		...
		switch (e.type) {
			...
			case "templateSql": return (e.chunkable ? $d(e.fragments, i, n) : [i]).map((s) => {
	...
	function Md(e, t, r) {
		...
		switch (n) {
			...
			case "parameterTupleList": return e.value.map((i) => {
	...
	function Ra(e, t, r) {
		if (!t.every((n) => ei(e, n))) throw new N(qd(e, r), jd(r), r.context);
	}
	function ei(e, t) {
		switch (t.type) {
		...
		async interpretNode(t, r) {
			switch (t.type) {
				...
				case "join": {
					...
					return {
						value: Ud(n, await Promise.all(t.args.children.map(async (s) => ({
							joinExpr: s,
							childRecords: (await this.interpretNode(s.child, r)).value
						}))), t.args.canAssumeStrictEquality),
						lastInsertId: i
					};
				}
				case "transaction": {
	...
	var ni = class {
		...
		#a() {
			return {
				inputNodeCount: this.#n(),
				outputNodeCount: this.#n(),
				rootCount: this.#n()
			};
		}
		#l(t) {
	...
	var Or = class {
		...
		#a(t, r) {
			if (ne(r, re.ParamEnum) && r.enumNameIndex !== void 0 && typeof t == "string") {
				...
				if (o && Object.hasOwn(o, t)) return this.#i(o[t], { type: "Enum" });
			}
		...
		#u(t, r, n) {
			if (ne(n, re.ParamScalar) && we(n) & M.Json) {
				let i = K(Z(t));
				return this.#i(i, { type: "Json" });
			}
			if (ne(n, re.ParamEnum)) {
				let i = this.#e.enumValues(n);
				if (i && t.every((o) => typeof o == "string" && Object.hasOwn(i, o))) return this.#i(t, {
					type: "List",
					inner: { type: "Enum" }
				});
			}
	...
	var nm = 100, et = _("prisma:client:transactionManager"), im = () => ({
		...
		async #r(t) {
			...
			switch (r.transaction = await Promise.race([o.finally(() => clearTimeout(i)), Ba(n.signal, "abort").then(() => {})]), this.transactions.set(r.id, r), r.status) {
				case "waiting":
...
`);
	...
	function el({ version: e, binaryTarget: t, title: r, description: n, engineVersion: i, database: o, query: s }) {
		let l = Za(De(Di(6e3 - (s?.length ?? 0)))), u = n ? `# Description
\`\`\`
${n}
\`\`\`` : "";
		return `${r}
...
${st(Xa({
			title: r,
			body: De(`Hi Prisma Team! My Prisma Client just crashed. This is the report:
## Versions
...
`;
	...
	function rl(e, t) {
		let r = (e || "").split(";").filter((l) => typeof l == "string" && !!l.trim()), i = pm(r.shift() || ""), o = i.name, s = i.value;
		try {
	...
	function il(e) {
		...
		return {
			apiKey: o,
			url: new URL(r.href.replace(n, s))
		};
	}
	...
	var ci = require_dist(), ui = {}, ll = { async loadQueryCompiler(e) {
...
${n.backtrace}`, { clientVersion: this.config.clientVersion });
		...
		#d(t) {
			return t instanceof F.PrismaClientRustPanicError ? t : typeof t.message == "string" && typeof t.code == "string" ? new F.PrismaClientKnownRequestError(t.message, {
				...
				meta: t.meta,
				clientVersion: this.config.clientVersion
			}) : typeof t.message == "string" ? new F.PrismaClientUnknownRequestError(t.message, { clientVersion: this.config.clientVersion }) : t;
		}
		#m(t, r, n = !0) {
		...
		#y(t) {
			switch (t) {
				...
				default: throw new F.PrismaClientKnownRequestError(`Inconsistent column data: Conversion failed: Invalid isolation level \`${t}\``, {
					code: "P2023",
					clientVersion: this.config.clientVersion,
...
`);
	...
	}.version.split(".")[0], wm = "PRISMA_INSTRUMENTATION", bm = `V${hm}_PRISMA_INSTRUMENTATION`, Cl = globalThis;
	function Rl() {
		let e = Cl[bm];
	...
	var Tm = _("prisma:client:request_handler"), zr = class {
		...
		handleRequestError({ error: t, clientMethod: r, callsite: n, transaction: i, args: o, modelName: s, globalOmit: a }) {
			...
			if (t instanceof b.PrismaClientKnownRequestError && Am(t)) ar({
				args: o,
				errors: [Fl(t.meta)],
				callsite: n,
		...
		unpack(t, r, n) {
			...
			let i = Object.keys(t)[0], o = Object.values(t)[0], a = wn(o, r.filter((u) => u !== "select" && u !== "include")), l = i === "queryRaw" ? Hr(a) : Z(a);
			return n ? n(l) : l;
		}
		get [Symbol.toStringTag]() {
...
Read more at https://pris.ly/d/client-constructor`), this.name = "PrismaClientConstructorValidationError";
	...
	var _l = [
		...
		errorFormat: (e) => {
			if (e) {
				...
				if (!$l.includes(e)) throw new S(`Invalid errorFormat ${e} provided to PrismaClient constructor.${Ut(e, $l)}`);
			}
		},
		log: (e) => {
			if (!e) return;
			...
			function t(r) {
				if (typeof r == "string" && !Ll.includes(r)) throw new S(`Invalid log level "${r}" provided to PrismaClient constructor.${Ut(r, Ll)}`);
			}
			for (let r of e) {
				t(r);
				let n = {
	...
	function jl(e, t) {
		for (let [r, n] of Object.entries(e)) {
			if (!_l.includes(r)) throw new S(`Unknown property ${r} provided to PrismaClient constructor.${Ut(r, _l)}`);
			Cm[r](n, t);
		}
		Rm(e);
	}
		...
		class t {
			...
			constructor(n) {
				...
				try {
					...
					if (l && _.enable("prisma:client"), s.errorFormat ? this._errorFormat = s.errorFormat : process.env.NODE_ENV === "production" ? this._errorFormat = "minimal" : process.env.NO_COLOR ? this._errorFormat = "colorless" : this._errorFormat = "colorless", this._runtimeDataModel = e.runtimeDataModel, this._engineConfig = {
						enableDebugLogs: l,
...
`, e.clientVersion);
			...
			async _transactionWithCallback({ callback: n, options: i = {} }) {
				...
				l.push(u);
				let c = { traceparent: this._tracingHelper.getTraceParent() }, p = {
					maxWait: i?.maxWait ?? this._engineConfig.transactionOptions.maxWait,
					timeout: i?.timeout ?? this._engineConfig.transactionOptions.timeout,
					isolationLevel: i?.isolationLevel ?? this._engineConfig.transactionOptions.isolationLevel,
					newTxId: i.newTxId
			...
			async _executeRequest({ args: n, clientMethod: i, dataPath: o, callsite: s, action: a, model: l, argsMapper: u, transaction: c, unpacker: p, otelParentCtx: d, customDataProxyFetch: m }) {
				try {
					n = u ? u(n) : n;
					let x = this._tracingHelper.runInChildSpan({ name: "serialize" }, () => pr({
						modelName: l,
						runtimeDataModel: this._runtimeDataModel,
						action: a,
						args: n,
...
`)), c?.kind === "batch" && await c.lock, this._requestHandler.request({
	...
	var b = require_dist(), oe = require_dist(), O = require_dist(), Xl = require_dist();
})))());
var config = {
	"previewFeatures": [],
	"clientVersion": "7.8.0",
...
config.runtimeDataModel = JSON.parse("{\"models\":{\"Task\":{\"fields\":[{\"name\":\"id\",\"kind\":\"scalar\",\"type\":\"Int\"},{\"name\":\"text\",\"kind\":\"scalar\",\"type\":\"String\"},{\"name\":\"completed\",\"kind\":\"scalar\",\"type\":\"Boolean\"}],\"dbName\":null}},\"enums\":{},\"types\":{}}");
config.parameterizationSchema = {
	strings: JSON.parse("[\"where\",\"Task.findUnique\",\"Task.findUniqueOrThrow\",\"orderBy\",\"cursor\",\"Task.findFirst\",\"Task.findFirstOrThrow\",\"Task.findMany\",\"data\",\"Task.createOne\",\"Task.createMany\",\"Task.createManyAndReturn\",\"Task.updateOne\",\"Task.updateMany\",\"Task.updateManyAndReturn\",\"create\",\"update\",\"Task.upsertOne\",\"Task.deleteOne\",\"Task.deleteMany\",\"having\",\"_count\",\"_avg\",\"_sum\",\"_min\",\"_max\",\"Task.groupBy\",\"Task.aggregate\",\"AND\",\"OR\",\"NOT\",\"id\",\"text\",\"completed\",\"equals\",\"not\",\"in\",\"notIn\",\"lt\",\"lte\",\"gt\",\"gte\",\"contains\",\"startsWith\",\"endsWith\",\"set\",\"increment\",\"decrement\",\"multiply\",\"divide\"]"),
...
config.compilerWasm = {
	getRuntime: async () => await Promise.resolve().then(() => require("./query_compiler_fast_bg.sqlite-Dz1R7zc6.js")),
	getQueryCompilerWasmModule: async () => {
		const { wasm } = await Promise.resolve().then(() => require("./query_compiler_fast_bg.sqlite.wasm-base64-D6CGWMUx.js"));
		return await decodeBase64AsWasm(wasm);
	},
	importName: "./query_compiler_fast_bg.js"
};
function getPrismaClientClass() {
	return import_client.getPrismaClient(config);
}
import_client.Extensions.getExtensionContext;
import_client.NullTypes.DbNull, import_client.NullTypes.JsonNull, import_client.NullTypes.AnyNull;
import_client.makeStrictEnum({ Serializable: "Serializable" });
import_client.Extensions.defineExtension;
//#endregion
//#region src/prisma/generated/client.ts
...
var prisma = new (getPrismaClientClass())({ adapter: new PrismaBetterSqlite3AdapterFactory({ url: "file:" + path.default.join(__dirname, "..", "..", "todo.db") }) });
function createWindow() {
	...
	}).loadFile(path.default.join(__dirname, "..", "..", "renderer", "dist", "renderer-app", "browser", "index.html"));
}
electron.ipcMain.handle("todo:getAll", async () => {
	return await prisma.task.findMany();
});
electron.ipcMain.handle("todo:add", async (_event, text) => {
	return await prisma.task.create({ data: {
		text,
		completed: false
	} });
});
electron.ipcMain.handle("todo:update", async (_event, id, completed) => {
	try {
...
electron.ipcMain.handle("todo:delete", async (_event, id) => {
	try {
